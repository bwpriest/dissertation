...
\begin{figure}
	\begin{center}
		...
		\end{subfigure}
		\caption{Weak and strong scaling wall time experiments for degree counting, as the number of nodes varies from 1 up to 1024.
...
\begin{figure}
	\begin{center}
		...
		\end{subfigure}
		\caption{Weak and strong scaling wall time experiments for connected components counting, as the number of nodes varies from 1 up to 1024.
...
In a pathological stream $E$ could hold as many as $r-1$ elements, i.e. there is one holdout register index that prevents $b$ from growing in spite of cut insertions. 
...
\begin{enumerate}
	\item \textbf{Estimation Quality} Do the algorithms yield good global and edge- and vertex-local estimates? How is their variance? 
	...
	\item \textbf{Speed} How fast is accumulation? Estimation? How does speed relate to $m = 2^p$?
	\item \textbf{Scalability} How does wall time relate to $|\mathcal{P}|$?
\end{enumerate}
...
\begin{figure}
	...
	\caption{Precision vs. recall and mean relative error (MRE) for the top 10, top 100, and top 1000 ground truth heavy hitters of all graphs listed in Table~\ref{tab:graphs} using $p=12$.
...
This graph proves to be a pathological for this problem. 
Note that, save the top 2 edges, the top $\sim 300$ edges are tied in triangle count.
Although our algorithm is able to reliably recover these top 2 elements, even an algorithm with prefect accuracy would fail up to ties on this graph for $k >2$ and $k < \sim 300$, which our results reflect.
...
\begin{figure}
	...
	Notably, the two edges with increased 
	\label{fig:hh_comp}}
\end{figure}
...
Unfortunately, in practice we typically cannot know how many outliers there are in practice.
...
We perform a similar comparison using Vigna's additive hyperbolic weighted $\tau_{h, \phi}$, an extension to the well-studied Kendall's $\tau$ statistic \cite{vigna2015weighted}.
...
\end{equation}
%
This coefficient yields a value $-1 \leq \tau_h \leq 1$, where $\tau_h = 1$ indicates perfect agreement between the indexings.
...
\phi_k(z) \coloneqq 
\begin{cases}
	i &\text{if $i \leq k$ and $z$ is the $i$th largest element}\\
	\infty &\text{else}
\end{cases},
\end{equation}
%
using $\tau_{h,\phi_k}$ to measure top $k$ correlation.
We used $k = 100$ in our experiments.
...
\hline
\textbf{or $\boldsymbol{\otimes}$ or} & \#\#\# & \#\#\# & \#\#\# & Kronecker \\
\hline
Twitter & 41,652,224 & \#\#\# &  \#\#\# & Soc. Net. \\
\hline
WebData & 3,563,602,788 & \#\#\# &  \#\#\# & Web \\
\hline
\end{tabular}
\end{center}
\end{table}
