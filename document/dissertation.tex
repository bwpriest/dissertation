...
\begin{onehalfspace}
\begin{center}
\textbf{Sublinear Approximation of Centrality Indices in Massive Graphs}
\vspace{0.4cm}
...
At a high level, so-called precision sampling involves executing reservoir sampling with an internal \algoname{CountSketch} data structure that estimates the frequency vector whose indices are modulated by the aforementioned random values. 
 


It is further worth noting that reservoir sampling can also be use to sample $t$ items \emph{with replacement} by maintaining $t$ parallel reservoir samplers each of size 1. 
