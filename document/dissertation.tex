...
We achieve our results by way of counting sketches, sampling sketches, and subspace embeddings.
...
%Assume that $t = O(\log n/\delta)$ and $r = k$. 
%Let $n_q = |v_{i_q}|$, where $v_{i_q}$ is the $q$th largest index of $v$. 
%Then there exists a streaming algorithm maintaining a heap during accumulation of \algoname{CountSketch} that can solve \algoname{ApproxTopCentral}$(\textrm{DC}, G, k, \varepsilon)$ using space
%%
%\begin{equation} \label{eq:dc:space}
%O \left (
%k \log \frac{n}{\delta}
...
%Note that the space of the above \algoname{ApproxTopCentral}$(\textrm{DC}, G, k, \varepsilon)$ algorithm depends on a quotient involving the valency of the bottom $n-k+1$ indices. 
...
%If $\alpha$ is chosen to be very small, then this problem will sort itself out. 
%However, 
...
%We may take some solace from the fact that sublinear analysis of dynamic graphs is still a very young field, and improved methods may arise in the near future. 
...
%Thus, this algorithm solves $\algoname{ApproxCentral}(\textrm{HITS},G,\nu)$, where $\nu$ is set equal to the right side of Equation (\ref{eq:hitsapprox}). 
\end{corollary}
...
%Indeed, it is known that the worst-case complexity of computing $\textrm{BC}(x)$ for $x \in V$ is $O(nm)$, no better than computing $BC(G)$. 
