...
However, we may not know which, if any of $\left \{ v_0, v_1, \dots, v_{i-1} \right \}$ are actually adjacent to $v_0$. 
If $\left \{ v_0, v_1, \dots, v_{i-1} \right \} \not \subseteq \mathcal{N}_\mathcal{S}$, then we are unable to flip a coin with probability 
$\frac{|\mathcal{N}_\mathcal{S} \setminus \left \{ v_0, v_1, \dots, v_{i-1} \right \}|}
...
This algorithm is much more efficient than other attempts to approximate betweenness centrality, as it sidesteps the need to approximate the \algoname{AllPairsAllShortestPaths} problem.
...
Algorithm~\ref{alg:kpath} runs in $O(\kappa^3 n^{2-2\alpha} \log n)$ time and $O(m)$ space.
