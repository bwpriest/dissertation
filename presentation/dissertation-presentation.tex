...
\title[Sublinear Centrality]{Sublinear-Space Approximations of Centrality in Evolving Massive Graphs} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\begin{enumerate}
	...
	\item {\color<3-6>{gray}Introduction \& Background}
	\only<2>{
	\begin{enumerate}
	...
	\item {\color<2, 4-6>{gray}Summary of Results}
	\only<3>{
	\begin{enumerate}
		\setlength\itemsep{0.5em}
	...
	\item {\color<2-3,5-6>{gray}Pseudo-Asynchronous Communication for Distributed Algorithms}
	\only<4>{
	\begin{enumerate}
		\setlength\itemsep{0.5em}
		\item Vertex-Centric Algorithm Challenges
		\item Pseudo-Asyncronous Communication Protocols
		\item Verification \& Implementation Details
	\end{enumerate}
	}
	\item {\color<2-4, 6>{gray}DegreeSketch and Local Triangle Count Heavy Hitters}
	\only<5>{
	\begin{enumerate}
		\setlength\itemsep{0.5em}
	...
	\item {\color<2-5>{gray}Sublinear $\kappa$-Path Centrality}
	\only<6>{
	\begin{enumerate}
		\setlength\itemsep{0.5em}
		...
		\item Sublinear Distributed $\kappa$-Path Centrality
	\end{enumerate}
	}
\end{enumerate}
}
...
\begin{itemize}
	...
	\begin{itemize}
		\item Including distributed generalizations \cite{shin2018tri, shin2018dislr}
	\end{itemize}
\end{itemize}
}
\end{dynblock}
...
\footnote{\tiny Benjamin W. Priest, Roger Pearce and Geoffrey Sanders, ``\algoname{DegreeSketch}: Distributed Cardinality Sketches on Graphs, with Applications to Counting Triangles,'' In preparation for SigKDD 2019}
}
...
\opaqueblock<2>{
\begin{itemize}
	\item	We show distributed sublinear vertex-centric random walk and simple path sampling algorithms
...
\begin{center}
\only<1>{
		\includegraphics[width=0.85\textwidth]{query_1}
}
\only<2>{
		\includegraphics[width=0.85\textwidth]{query_2}
}
\only<3>{
		\includegraphics[width=0.85\textwidth]{query_3}
}
\end{center}
...
\begin{frame}
\frametitle{High vs Low Triangle Density}

\centerline{\includegraphics[width=0.75\columnwidth]{hh_comp}}
...
\begin{itemize}
	...
	\begin{itemize}
		\item e.g. 2nd Neighborhood size
	\end{itemize}
\end{itemize}
